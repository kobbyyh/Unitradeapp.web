// Unified Notification Service for Unitrade
// Single source of truth for all notification functionality

class UnifiedNotificationService {
    constructor() {
        this.isInitialized = false;
        this.db = null;
        this.init();
    }

    async init() {
        try {
            // Use modern Firebase v9+ imports
            const { initializeApp } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-app.js');
            const { getFirestore } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
            
            const firebaseConfig = {
                apiKey: "AIzaSyBzyKtEKbUMt66t9Sfk_onbOcJNic_t5oc",
                authDomain: "unitrade-d74e9.firebaseapp.com",
                projectId: "unitrade-d74e9",
                storageBucket: "unitrade-d74e9.firebasestorage.app",
                messagingSenderId: "619229942843",
                appId: "1:619229942843:web:8eced8b46057a24b04f81e",
                measurementId: "G-6DPM2BCY97"
            };

            this.app = initializeApp(firebaseConfig);
            this.db = getFirestore(this.app);
            this.isInitialized = true;
            
            console.log('✅ Unified Notification Service initialized');
        } catch (error) {
            console.error('❌ Error initializing notification service:', error);
            throw error;
        }
    }

    // Create seller notification
    async createSellerNotification(orderData, sellerId) {
        if (!this.isInitialized) {
            await this.init();
        }

        try {
            const { collection, addDoc, serverTimestamp } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
            
            const notificationData = {
                sellerId: sellerId,
                orderId: orderData.id,
                type: 'new_order',
                title: '🎉 New Order Received!',
                message: `${orderData.buyerName || 'A buyer'} ordered "${orderData.productTitle || 'your item'}" for GHS ${orderData.totalPrice || orderData.productPrice || '0.00'}`,
                orderData: {
                    productTitle: orderData.productTitle || 'Unknown Product',
                    productPrice: orderData.productPrice || 0,
                    quantity: orderData.quantity || 1,
                    totalPrice: orderData.totalPrice || orderData.productPrice || 0,
                    buyerName: orderData.buyerName || 'Unknown Buyer',
                    buyerPhone: orderData.buyerPhone || orderData.buyerContact || 'Not provided',
                    buyerLocation: orderData.buyerLocation || orderData.shippingAddress || 'Not provided',
                    orderDate: new Date()
                },
                read: false,
                createdAt: serverTimestamp(),
                priority: 'high'
            };

            const docRef = await addDoc(collection(this.db, 'seller_notifications'), notificationData);
            
            console.log('✅ Seller notification created:', docRef.id);
            return { success: true, notificationId: docRef.id };
            
        } catch (error) {
            console.error('❌ Error creating notification:', error);
            console.error('❌ Error details:', {
                message: error.message,
                code: error.code,
                stack: error.stack
            });
            throw new Error('Failed to create notification: ' + error.message);
        }
    }

    // Get seller notifications
    async getSellerNotifications(sellerId) {
        if (!this.isInitialized) {
            await this.init();
        }

        try {
            const { collection, query, where, orderBy, limit, getDocs } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
            
            const notificationsSnapshot = await getDocs(
                query(
                    collection(this.db, 'seller_notifications'),
                    where('sellerId', '==', sellerId),
                    orderBy('createdAt', 'desc'),
                    limit(50)
                )
            );

            const notifications = [];
            notificationsSnapshot.forEach((doc) => {
                notifications.push({
                    id: doc.id,
                    ...doc.data()
                });
            });

            return notifications;
        } catch (error) {
            console.error('❌ Error getting notifications:', error);
            throw new Error('Failed to get notifications: ' + error.message);
        }
    }

    // Mark notification as read
    async markNotificationAsRead(notificationId) {
        if (!this.isInitialized) {
            await this.init();
        }

        try {
            const { doc, updateDoc, serverTimestamp } = await import('https://www.gstatic.com/firebasejs/10.7.1/firebase-firestore.js');
            
            await updateDoc(doc(this.db, 'seller_notifications', notificationId), {
                read: true,
                readAt: serverTimestamp()
            });

            return { success: true };
        } catch (error) {
            console.error('❌ Error marking notification as read:', error);
            throw new Error('Failed to mark notification as read: ' + error.message);
        }
    }

    // Get unread count
    async getUnreadCount(sellerId) {
        try {
            const notifications = await this.getSellerNotifications(sellerId);
            return notifications.filter(n => !n.read).length;
        } catch (error) {
            console.error('❌ Error getting unread count:', error);
            return 0;
        }
    }

    // Send WhatsApp notification using Twilio
    async sendWhatsAppNotification(sellerPhoneNumber, orderData) {
        try {
            console.log('📱 Sending Twilio WhatsApp notification to:', sellerPhoneNumber);
            
            // Check if Twilio WhatsApp service is available
            if (typeof twilioWhatsAppService !== 'undefined') {
                const result = await twilioWhatsAppService.sendOrderNotification(sellerPhoneNumber, orderData);
                console.log('✅ Twilio WhatsApp notification sent successfully');
                return result;
            } else {
                console.log('⚠️ Twilio WhatsApp service not available');
                return { success: false, message: 'Twilio WhatsApp service not available' };
            }
            
        } catch (error) {
            console.error('❌ Error sending WhatsApp notification:', error);
            throw new Error('Failed to send WhatsApp notification: ' + error.message);
        }
    }

    // Send email notification using OneSignal
    async sendEmailNotification(orderData, sellerEmail) {
        try {
            console.log('📧 Sending OneSignal email notification to:', sellerEmail);
            
            // Check if OneSignal email service is available
            if (typeof oneSignalEmailService !== 'undefined') {
                const result = await oneSignalEmailService.sendOrderNotificationEmail(sellerEmail, orderData);
                console.log('✅ OneSignal email sent successfully');
                return result;
            } else {
                // Fallback to preview if OneSignal not available
                console.log('⚠️ OneSignal not available, showing email preview');
                const emailContent = this.generateEmailContent(orderData);
                this.showEmailPreview(orderData, sellerEmail, emailContent);
                return { success: true, message: 'Email preview shown (OneSignal not available)' };
            }
            
        } catch (error) {
            console.error('❌ Error sending email notification:', error);
            // Fallback to preview on error
            try {
                const emailContent = this.generateEmailContent(orderData);
                this.showEmailPreview(orderData, sellerEmail, emailContent);
                return { success: true, message: 'Email preview shown (OneSignal failed)' };
            } catch (previewError) {
                throw new Error('Failed to send email notification: ' + error.message);
            }
        }
    }

    // Generate email content
    generateEmailContent(orderData) {
        return `
📧 NEW ORDER NOTIFICATION
═══════════════════════════════════════════════════════════════

Dear Seller,

🎉 You have received a new order on Unitrade!

ORDER DETAILS:
───────────────────────────────────────────────────────────────
Product: ${orderData.productTitle || 'Product'}
Price: GHS ${orderData.productPrice || '0.00'}
Quantity: ${orderData.quantity || 1}
Total Amount: GHS ${(orderData.totalPrice || orderData.productPrice || 0) * (orderData.quantity || 1)}
Order ID: ${orderData.id || 'Unknown'}
Order Date: ${new Date().toLocaleDateString()}

BUYER INFORMATION:
───────────────────────────────────────────────────────────────
Name: ${orderData.buyerName || 'Unknown'}
Phone: ${orderData.buyerPhone || orderData.buyerContact || 'Not provided'}
Location: ${orderData.buyerLocation || orderData.shippingAddress || 'Not provided'}

NEXT STEPS:
───────────────────────────────────────────────────────────────
1. Log in to your seller dashboard
2. Contact the buyer using the information above
3. Arrange pickup or delivery
4. Update order status when completed

This notification was sent from Unitrade - University Trading Platform
═══════════════════════════════════════════════════════════════
        `;
    }

    // Show email preview
    showEmailPreview(orderData, sellerEmail, emailContent) {
        const modal = document.createElement('div');
        modal.className = 'fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50';
        modal.innerHTML = `
            <div class="bg-white rounded-xl p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto">
                <div class="flex justify-between items-center mb-4">
                    <h3 class="text-xl font-semibold">📧 Email Notification</h3>
                    <button onclick="this.closest('.fixed').remove()" class="text-gray-500 hover:text-gray-700">
                        <i class="fas fa-times text-xl"></i>
                    </button>
                </div>
                <div class="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                    <p class="text-blue-800 font-medium">📧 Email would be sent to: ${sellerEmail}</p>
                    <p class="text-blue-700 text-sm">To send real emails, integrate with EmailJS or your preferred email service.</p>
                </div>
                <div class="bg-gray-100 p-4 rounded-lg">
                    <pre class="text-sm whitespace-pre-wrap font-mono">${emailContent}</pre>
                </div>
                <div class="mt-4 flex justify-end space-x-2">
                    <button onclick="this.closest('.fixed').remove()" class="bg-gray-500 text-white px-4 py-2 rounded-lg hover:bg-gray-600">
                        Close
                    </button>
                    <button onclick="this.closest('.fixed').remove(); alert('Email sent! (This is a preview)')" class="bg-teal-600 text-white px-4 py-2 rounded-lg hover:bg-teal-700">
                        Send Email
                    </button>
                </div>
            </div>
        `;
        document.body.appendChild(modal);
    }
}

// Initialize and export
const unifiedNotificationService = new UnifiedNotificationService();
window.unifiedNotificationService = unifiedNotificationService;
